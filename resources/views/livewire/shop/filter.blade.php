    ...
    <x-container class="md:flex px-4">
            ...
            <aside class="md:w-52 md:flex-shrink-0 md:mr-8 mb-4 md:mb-0">
                <ul class="space-y-4">
                    @foreach ($options as $option)
                        <li x-data="{ open: true }">
                            <button
                                class="px-4 py-2 w-full text-left flex justify-between items-center bg-gray-100 text-black dark:bg-gray-700 dark:text-white font-medium"
                                x-on:click="open = !open">
                                {{ $option['name'] }}
                                <i class="fa-solid"
                                    :class="open ? 'fa-angle-down' : 'fa-angle-up'"></i>
                            </button>

                            <ul class="mt-3 space-y-1" x-show="open" x-transition>
                                    ...
                                    <li>
                                        <label class="inline-flex items-center text-gray-800 dark:text-gray-200">
                                            <x-checkbox
                                                class="mr-2"
                                                value="{{ $feature['id'] }}"
                                                wire:model.live="select_features"
        ...
        <div class="md:flex-1">
            <div class="flex items-center mb-4">
                ...
                <x-select wire:model.live="orderBy" class="w-48">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mx-6 sm:mx-0">
                @forelse ($products as $product)
                    @if ($product->variants->isNotEmpty())
                        @php
                            $variant = $product->variants->first();
                            $imagePath = optional($variant->images->first())->path;
                        @endphp
                        ...
                        <article class="bg-white border-gray-200 dark:bg-gray-800 dark:border-gray-700 shadow rounded overflow-hidden relative group">
                            <a href="{{ route('products.show', $product) }}">
                                <img
                                    src="{{ $imagePath ? Storage::url($imagePath) : asset('images/default.png') }}"
                                ...
                                <div class="p-4">
                                    ...
                                    <p class="text-gray-900 dark:text-gray-200 mb-4">
                                        S/. {{ number_format($variant->price, 2) }}
                                    </p>
                    ...
                    @endif
                @empty
                    <p class="text-gray-700 dark:text-gray-300 col-span-full">No se encontraron productos.</p>
                @endforelse
            </div>
